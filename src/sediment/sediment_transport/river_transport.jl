...
function SedimentRiverTransportParameters(dataset, config, indices)
    ...
    lens = lens_input_parameter("river_bottom-and-bank_clay__d50_diameter")
    dm_clay = ncread(dataset, config, lens; sel = indices, defaults = 2.0, type = Float)
    lens = lens_input_parameter("river_bottom-and-bank_silt__d50_diameter")
    dm_silt = ncread(dataset, config, lens; sel = indices, defaults = 10.0, type = Float)
    lens = lens_input_parameter("river_bottom-and-bank_sand__d50_diameter")
    dm_sand = ncread(dataset, config, lens; sel = indices, defaults = 200.0, type = Float)
    lens = lens_input_parameter("river_bottom-and-bank_aggregates~small__d50_diameter")
    dm_sagg = ncread(dataset, config, lens; sel = indices, defaults = 30.0, type = Float)
    lens = lens_input_parameter("river_bottom-and-bank_aggregates~large__d50_diameter")
    dm_lagg = ncread(dataset, config, lens; sel = indices, defaults = 500.0, type = Float)
    lens = lens_input_parameter("river_bottom-and-bank_gravel__d50_diameter")
    dm_gravel =
...
function SedimentConcentrationsRiverParameters(dataset, config, indices)
    lens = lens_input_parameter("river_water_clay__d50_diameter")
    dm_clay = ncread(dataset, config, lens; sel = indices, defaults = 2.0, type = Float)
    lens = lens_input_parameter("river_water_silt__d50_diameter")
    dm_silt = ncread(dataset, config, lens; sel = indices, defaults = 10.0, type = Float)
    lens = lens_input_parameter("river_water_sand__d50_diameter")
    dm_sand = ncread(dataset, config, lens; sel = indices, defaults = 200.0, type = Float)
    lens = lens_input_parameter("river_water_aggregates~small__d50_diameter")
    dm_sagg = ncread(dataset, config, lens; sel = indices, defaults = 30.0, type = Float)
    lens = lens_input_parameter("river_water_aggregates~large__d50_diameter")
    dm_lagg = ncread(dataset, config, lens; sel = indices, defaults = 500.0, type = Float)
    lens = lens_input_parameter("river_water_gravel__d50_diameter")
    dm_gravel =
        ncread(dataset, config, lens; sel = indices, defaults = 2000.0, type = Float)
