...
function initial(staticmaps_path, leafarea_path)
    ...
    dParams = Dict([
                        ("Cfmax",3.75653 * (timestepsecs/basetimestep)), ("TT",0.),
                        ("TTM",0.0), ("TTI",1.0),
                        ("WHC",0.1), ("cf_soil",0.038),
                        ("w_soil",0.1125 * (timestepsecs/basetimestep)),("SoilThickness",2000.0),
                        ("InfiltCapSoil",100.0), ("InfiltCapPath",10.0),
                        ("PathFrac",0.01), ("WaterFrac",0.0),
                        ("thetaS",0.6), ("thetaR",0.01),
                        ("AirEntryPressure",10.0), ("KsatVer",3000.0 * (timestepsecs/basetimestep)),
                        ("MaxLeakage",0.0), ("c",10.0),
                        ("M",300.0), ("CapScale",100.0),
                        ("rootdistpar",-500.0), ("RootingDepth",750.0),
                        ("LAI",1.0), ("et_RefToPot",1.0)
                        ])

    nc = NCDataset(staticmaps_path)

    subcatch_2d = "wflow_subcatch" in keys(nc) ? nc["wflow_subcatch"][:] : @error("wflow_subcatch not found")
    # indices based on catchment
    inds = Wflow.active_indices(subcatch_2d, missing)
    n = length(inds)

    Altitude = "wflow_dem" in keys(nc) ? readnetcdf(nc,"wflow_dem",inds,dParams) :  @error("wflow_dem not found")
    ...
    for i in 1:n
        params[i] = SBMParams(Cfmax=Cfmax[i],TT=TT[i],TTI=TTI[i],TTM=TTM[i],WHC=WHC[i],w_soil=w_soil[i],cf_soil=cf_soil[i],
                                thetaS=thetaS[i],thetaR=thetaR[i],KsatVer=KsatVer[i],M=M[i],AirEntryPressure=AirEntryPressure[i],
                                nLayers=nLayers,SoilThickness=SoilThickness[i],InfiltCapPath=InfiltCapPath[i],
                                InfiltCapSoil=InfiltCapSoil[i],MaxLeakage=MaxLeakage[i],WaterFrac=WaterFrac[i],
                                PathFrac=PathFrac[i],Altitude=Altitude[i],RootingDepth=RootingDepth[i],
                                rootdistpar=rootdistpar[i],CapScale=CapScale[i],et_RefToPot=et_RefToPot[i],
                                Sl=Sl[i],Swood=Swood[i],Kext=Kext[i],c=c[:,i],LAI=1.0
                                )
    end
