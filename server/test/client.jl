...
@testset "variable information and get and set functions" begin
    ...
    vwc_1_itemsize = request((fn="get_var_itemsize", name="vertical.vwc[1]"))["var_itemsize"]
    ...
    msg = (fn="set_value_at_indices", name="vertical.zi", src=[250.0, 350.0], inds=[1, 2])
    @test request(msg) == Dict("status" => "OK")
    msg = (
        fn="get_value_at_indices", name="vertical.zi", dest=[0.0, 0.0, 0.0], inds=[1, 2, 3]
    )
    ...
    msg = (fn="set_value_at_indices", name="vertical.vwc[1]", src=[0.1, 0.25], inds=[1, 2])
    @test request(msg) == Dict("status" => "OK")
    msg = (fn="get_value_at_indices", name="vertical.vwc[1]", dest=[0.0, 0.0], inds=[1, 2])
