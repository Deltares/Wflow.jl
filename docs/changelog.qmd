...
### Fixed
- Forcing that is spread over multiple netCDF files is now read one file at a time (#1037).
  Previously all files were kept open for the entire simulation, and since netCDF gives every
  variable of every open file its own chunk cache, this used up to `n_files * n_variables *
  64 MiB` of memory, which made runs with many forcing files run out of memory. Reading is
  also faster, since presenting the files as a single aggregated dataset added a large
  overhead to every read.
