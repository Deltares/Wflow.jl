...
The change of using drainable and dynamic fillable porosity for computing water table
changes (#730) instead of using the effective porosity (difference between saturated and
residual water content) can have a relatively large impact on model results, generally
resulting in higher peak flow, lower baseflow and higher actual evapotranspiration.

By default hydraulic heads are computed for the kinematic wave lateral subsurface flow
concept (#827). Land surface elevation data is required for this computation and can be set
as follows in the TOML configuration file:
...
Supporting internal timestepping for the kinematic wave lateral subsurface flow concept
(#827) requires that the flow state variable contains `instantaneous` in the `quantity` part
...
### Fixed
- Reduce memory use and improve performance of reading split NetCDF forcing files ([#1037](https://github.com/Deltares/Wflow.jl/pull/1052)).

### Changed
- All internal computations now use standardized SI units (m, s, kg, J, K) consistently
  throughout the codebase. Units are converted to/from the old units at I/O
  boundaries. This generally does not change model results (units are converted back before
  writing outputs), though small numerical differences may occur in some cases.
- The configuration TOML file written to `dir_output` at the end of a simulation now
  reflects the (potentially modified) in-memory configuration instead of being a copy of the
  original input TOML file.
- Improved kinematic wave solver performance by substituting `pow` calls with direct arithmetic
  on the 5th root of discharge, and computing cross-sectional area as a byproduct of the
...
- Drainable porosity or specific yield, defined as the difference between `theta_s`
  ...
  `UnconfinedAquifer`. For the change of a rising water table of these concepts a dynamic
  fillable porosity (based on `theta_s` and the water content of unsaturated soil layers) is
  used. This results in a better integration of the `SBM` soil model (unsaturated zone) with
  these subsurface flow concepts as specific yield is typically represented by drainable
  porosity for these concepts and not by the effective porosity (difference between
  saturated and residual water content) of the previous implementation. Water content at
  field capacity `theta_fc` is a new optional input parameter, when this parameter is not
  provided `theta_fc` is computed as the water content at a pressure head of -100 cm using
  the Brooks and Corey water retention model.

### Added
- Support internal timestepping for kinematic wave lateral subsurface flow concept
  `LateralSSF`.
- Support including river and drainage head-dependent flux boundary conditions for kinematic
  wave lateral subsurface flow concept `LateralSSF`.
- Support for running only a subsection of the model, either by using a smaller active
...
- An optional 1D floodplain schematization to kinematic wave routing of river flow. This is
  the same 1D floodplain schematization used by the local inertial river flow model. River
  channel-floodplain water exchange is assumed to be instantaneous. Routing is done
  separately for the river channel and floodplain. Floodplain water is routed down slope
  using the `accucapacityflux` function with a transport capacity based on Manning's
  equation.
- Support the use of Manning's equation (assuming that the topography controls water flow
  mostly) for river and floodplain routing on a staggered grid. This is an additional
  routing scheme to the local inertial river routing scheme, both make us of the staggered
  grid method.
...
- Optional reinfiltration of overland flow into the soil, enabled by setting
  `model.land_surface_water_reinfiltration__flag = true` in the TOML file. When enabled, up
  to 95% of the overland flow depth is added to the water available for infiltration, and
  overland flow water levels are corrected accordingly.
- Support surface ponding on the land surface for both kinematic wave and 2D local inertial
  overland flow, through an optional per-cell parameter
  `land_surface_water__ponding_depth`. Incoming water fills the pond up to the retention
  capacity before contributing to overland flow. When land surface water reinfiltration is
  enabled, the pond storage is drawn from before the flowable share of the overland flow
  depth.
...
- Allow spilling of reservoirs with a maximum storage when using reservoir observed outflow
  data. The observed outflow was only capped based on reservoir water availability while an
  upper bound on storage for reservoirs with a maximum storage was not applied. This has
  been fixed so reservoir storage does not exceed maximum storage.
- Removed incorrect check for floodplain `hydraulic_radius` (introduced in v1.0.3). The
  check is also not required as floodplain flow computation is based on a `flow_area`
  threshold.
- Documentation: fixed units for finite difference formulation for groundwater flow in one
  dimension. These are fixed by multiplying the $S$ and $Q$ terms with $\Delta x$ and
  $\Delta y$.
- Intercell conductance for unconfined aquifer with `exponential` `conductivity_profile`.
  The conductance was not scaled by using the “upstream saturated fraction” which uses the
  degree of saturation based on the saturated thickness of the cell with the highest head.

## v1.0.3 - 2026-06-23

### Fixed
- Fix unit external inflow for local inertial river (1D) and land (2D) routing. For the
  computation of available river volume (to limit negative external inflow) the
  `bankfull_depth` was assigned to this volume, this should be `bankfull_storage`.
- Maximum subsurface flow was not adjusted for sub-daily timestep for `layered` and
  `layered_exponential` profiles.
- Fix deriving active indices for output variable `to_river`. Also, the mapping of standard
  name (or model path) to active indices is now more strict (comparing prefix of these
  strings instead of substring matching).
- Fix `waterlevel` unit (from \[m\] to \[mm\]) in rainfall erosion computation of rainfall
  erosion model `EUROSEM`. With the wrong unit (\[m\]) the rainfall impact reduction when
  the soil is covered by water was underestimated.
- Removed a conversion factor of 0.001 used for the computation of total kinetic energy as
  part of rainfall erosion model `EUROSEM`, as it is not required based on the units.

## v1.0.2 - 2026-03-05

### Fixed
- Water mass balance computation for `LateralSSF` at a sub-daily model timestep. A unit
  conversion (to sub-daily timestep) of the `recharge` variable was missing and has been
  added.
- Fixed issue with where the reservoir outflow could become negative for very low reservoir
  storage values, due to floating point precision errors. The `storage_input` variable is
  now limited to a minimum of zero.
- Limiting kinematic wave subsurface flow that flows partly below the soil bottom while
  solving the kinematic wave has been fixed. The computation to limit subsurface flow was
  not correct resulting in an subsurface flow overestimation.
- A bug was fixed that caused Wflow to error when parameters in the TOML didn't have a
  specified `scale` and/or `offset` value.
- Add missing update of `total_storage` of `SbmSoilModel` for model type `sbm_gwf`.

## v1.0.1 - 2026-01-07

### Fixed
- Fixed mapping standard name `soil_surface_water_saturated_zone__exfiltration_volume_flux`
  to model variable, it was incorrectly mapped to variable `exfiltustore` and this has been
  changed to variable `exfiltsatwater`.
...
- For the computation of Gash interception model parameter `e_r` multiply the precipitation
 ...
 - The internal time step of the local inertial model (`stable_timestep` function) can get
  ...
  This issue occurred on a virtual machine, Windows 10 Enterprise, with Intel(R) Xeon(R) Gold
