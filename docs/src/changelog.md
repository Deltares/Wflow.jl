...
- External inflow to the [SBM + Local inertial river (1D) and land (2D)](@ref) model
  configuration.
- Add parameters to scale precipitation and potential evaporation (precipitation_scale and potential_evaporation_scale) and offset temperature (temperature_offset) to facilitate climate runs with delta change approach. 
