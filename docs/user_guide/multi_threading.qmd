...
inertial model for river flow](@ref config_sbm_gwf_lie_river_land) and the optional [local
inertial model for river (1D) and land (2D)](@ref config_sbm_gwf_lie_river_land), both part of
wflow\_sbm, can also run on multiple threads. The threading functionality for the kinematic
wave may also be useful for the wflow\_sbm model [SBM + Groundwater flow](@ref config_sbm_gwf).
The multi-threading functionality in wflow is considered experimental, see also the following
