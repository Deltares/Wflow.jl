...
static parameter standard names to the external netCDF variable names. It is possible to
provide cyclic parameters to the model (minimum time step of 1 day). In the example below,
the model parameter standard name `vegetation__leaf-area_index` is mapped to the external
netCDF variable "LAI". Cyclic time inputs of parameters can be different (e.g., daily or
...
more details in the [Output CSV section](#output-csv-section) section. If a `map` is
