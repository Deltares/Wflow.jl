...
(e.g., daily or monthly). The `time` dimension name of these cyclic input parameters in the
...
# Output locations: these are not directly part of the model
...
parameter = "land.soil.variables.diagnostic.volumetric_water_content"
layer = 2
...
For example, to set the input parameter `snowpack__degree_day_coefficient` to an uniform
value of $2.5$:
...
`soil_layer_water__brooks_corey_exponent` as follows:
