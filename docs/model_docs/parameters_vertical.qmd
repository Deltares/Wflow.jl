...
Different [vertical hydraulic conductivity depth profiles](@ref soil): `exponential` (default),
`exponential_constant`, `layered` and `layered_exponential` can be provided through the TOML
file. Below an example for the `exponential_constant` profile:
