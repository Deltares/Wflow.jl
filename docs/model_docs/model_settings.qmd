...
| `soil_infiltration_reduction` | Enable reduction factor applied to the soil infiltration capacity | - | `false` |
| `soil_thickness_layer` | Thickness of soil layers | mm | - |
| `topog_sbm_transfer` | Enable original Topog_SBM vertical transfer formulation  | - | `false` |
...
| `kinematic_wave_adaptive_tstep` | Enable kinematic wave adaptive (internal) time stepping  | - | `false` |
| `kinematic_wave_river_tstep` | Enable kinematic wave fixed internal time stepping (`kinematic_wave_adaptive_tstep` = `false`) | s | Δt |
| `kinematic_wave_land_tstep` | Enable kinematic wave fixed internal time stepping (`kinematic_wave_adaptive_tstep` = `false`) | s | Δt |
: {.striped .hover}

### Local inertial routing
| Setting | Description | Unit | Default |
| --- | ----- | - | - |
| `local_inertial_flow_alpha` | Stability coefficient for internal model time step (CFL condition) | - | 0.7 |
| `local_inertial_flow_theta` | Weighting factor that adjusts the amount of artificial numerical diffusion for local inertial overland flow | - | 0.8 |
| `waterdepth_threshold` | Water depth threshold for calculating flow between cells | m | 0.001 |
