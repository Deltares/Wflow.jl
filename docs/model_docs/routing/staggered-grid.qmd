...
It is possible to include a [1D floodplain
schematization](#optional-1d-floodplain-schematization) for this routing type.
...
As for the [local inertial model](#local-inertial-river-and-floodplain-routing), the
momentum equation is applied to each edge between two river nodes, while the continuity
equation over $\Delta t$ is applied to each river node.
