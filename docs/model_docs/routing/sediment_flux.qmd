...
where $\SIb{C_{\max}}{kg\ L^{-1}}$ (or $\SIb{}{ton\ m^{-3}}$) is the sediment concentration,
...
$\SI{1.0 \times 10^6}{g\ m^{-3}}$ and $\SI{2.65 \times 10^6}{g\ m^{-3}}$), $\SIb{u}{m\ s^{-1}}$ is the water mean
...
$$
   \psi = \dfrac{V^3}{\left(\dfrac{\rho_{s}}{\rho}-1\right) g h \omega_{s,50}  \left[ \log_{10}\left(\dfrac{h}{D_{50}}\right)\right]^{2}}
$$

for some velocity V (see the original publication by Molinas & Wu, this was copied from a wrong formula in the SWAT documentation).
...
(`reservoir_water_sediment__bedload_trapping_efficiency = 1.0`, for example for a gravity dam)
...
reservoir_water_sediment__bedload_trapping_efficiency = "ResTrapEff"
```
