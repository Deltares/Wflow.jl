...
celerity $c$ is a function of the water table depth $\subtext{z}{ssf, watertable}$ and for
these profiles the kinematic wave equation is solved using Newton's method with an iteration
criterion set to a maximum allowed water table depth change of $\SI{0.1}{m}$. This iteration
criterion is applied to each model cell independently to compute and route subsurface flow
for each model timestep (routing is not done at an internal timestep, see also the note
below).
