...
## Surface ponding
A per-cell surface ponding depth $\SIb{\subtext{h}{pond}}{m}$ can be provided as a static
parameter to represent retention of water on the land surface in small depressions. The
parameter defaults to $\SI{0}{m}$ (no retention) and can be provided as follows in the TOML
configuration file:

```toml
[input.static]
land_surface_water__ponding_depth = "ponding_depth"
```

For kinematic wave overland flow, incoming lateral and upstream inflow is first used to fill
pond storage $\SIb{\subtext{V}{pond}}{m^3}$ up to the retention capacity
$\SIb{\subtext{V}{pond,cap}}{m^3}$:

$$
\subtext{V}{pond,cap} = \subtext{h}{pond} \subtext{L}{flow} \subtext{w}{flow},
$$

where $\SIb{\subtext{L}{flow}}{m}$ is the flow length and $\SIb{\subtext{w}{flow}}{m}$ is
the surface flow width. The remaining pond capacity $\subtext{V}{pond,cap} -
\subtext{V}{pond}$ is absorbed from the inflow first, and only the inflow in excess of this
remaining capacity is passed on to the kinematic wave. The resulting pond depth
$\subtext{V}{pond} / (\subtext{L}{flow} \subtext{w}{flow})$ is added to the reported surface
water depth $\SIb{h}{m}$ but does not drive the downstream discharge $\SIb{Q}{m^3\ s^{-1}}$.

The same `ponding_depth` parameter is used by the [local inertial overland flow (2D)
routing](./staggered-grid.qmd#surface-ponding). When [land surface water
reinfiltration](../land/landhydrology_sbm.qmd#land-surface-water-infiltration) is enabled,
the pond storage is drawn from before the flowable share of the overland flow depth.

## Floodplain routing
It is possible to include a [floodplain
schematization](./staggered-grid.qmd#floodplain-schematization) as part of the kinematic
wave river flow routing, using the following setting in the TOML file:
...
At the start of each time step, channel-floodplain water exchange, assumed to be
instantaneous, is computed based on total water volume and the channel storage capacity. The
exchange is added to the lateral inflow term of the kinematic wave equation. Routing is done
separately for the river channel and floodplain. Floodplain water is routed down slope using
the `accucapacityflux` function with a transport capacity based on Manning's equation:

$$
Q_{t+\Delta t} = n^{-1} R_t^{2/3} \sqrt{c_\mathrm{floodplain\ slope}} A_t
$$

where $\SIb{Q_{t+\Delta t}}{m^3\ s^{-1}}$ is the floodplain flow at time step $t+\Delta t$,
$\SIb{n}{m^{-\frac{1}{3}}\ s}$ is the Manning's roughness coefficient, $\SIb{R_t}{m}$ is the
hydraulic radius at the previous time step,  $c_\mathrm{floodplain\ slope}$ is the
floodplain bed slope and $\SIb{A_t}{m^2}$ is the cross sectional flow area at the previous
time step.
...
$$
    (\theta_s-\theta_{fc})w\frac{\partial h}{\partial t} = -\frac{\partial \subtext{Q}{subsurface}}{\partial x} + wR
$$
...
$\SIb{\theta_{fc}}{-}$ are the saturated soil water content and soil water content at field
capacity, respectively. The difference between $\theta_s$ and $\theta_{fc}$ represents the
drainable porosity or specific yield and is assumed to be constant. Substituting for
$h(\frac{\partial Q}{\partial h})$, gives:
...
$$
    c = \frac{\subtext{K}{h} c_\mathrm{land\ slope}}{\theta_s-\theta_{fc}}.
$$

The water table depth $\subtext{z}{ssf, watertable}$ is based on the change in subsurface
storage and the drainable porosity for a falling water table and the fillable porosity for a
rising water table, see also the following
[section](..\land\landhydrology_sbm.qmd#water-table-change). Using the fillable porosity for
a rising water table prevents mass balance errors for the entire soil (unsaturated and
saturated zone) when the fillable porosity is not equal to the drainable porosity used for
the computation of lateral subsurface flow.

The kinematic wave equation for lateral subsurface flow is solved iteratively using Newton's
method until a stable solution is reached. The internal time stepping is based on the
Courant number or a fixed time step size $\SIb{}{s}$. To enable a fixed internal time step
for the kinematic wave the following lines can be inserted in the TOML file of the model:

```toml
[model]
# Adaptive (internal) time stepping for the kinematic wave
kinematic_wave__adaptive_time_step_flag = false # optional, default is false
# Fixed internal timestep for subsurface flow
subsurface_kinematic_wave__time_step = 43200 # optional, default is 86400
```

Kinematic wave adaptive (internal) timestepping can be set as follows in the TOML file of
the model, including an alpha coefficient to enhance the stability of the simulation:

```toml
[model]
# Adaptive (internal) time stepping for the kinematic wave
kinematic_wave__adaptive_time_step_flag = true    # optional, default is false
subsurface_kinematic_wave__alpha_coefficient = 0.5 # optional, default is 1.0
```

When head-dependent flux [boundaries](#boundary-conditions) are included, we recommend to
check for possible numerical instabilities, especially when fluxes from these boundaries
(e.g. river) can be large, and to change the alpha coefficient accordingly. Otherwise, the
default value of 1.0 for the alpha coefficient is typically used for a stable simulation.

For the `exponential` and `exponential_constant` profiles, the kinematic wave celerity $c$
is a function of the water table depth $\subtext{z}{ssf, watertable}$ and for these profiles
the kinematic wave equation is solved using Newton's method with an iteration criterion set
to a maximum allowed water table depth change of $\SI{0.1}{m}$. This iteration criterion is
applied to each model cell independently to compute and route subsurface flow for each
(internal) model timestep.

### Boundary conditions
Two head-dependent flux boundary conditions that are also used by the [groundwater
flow](gwf.qmd#) concept, the [river](gwf.qmd#river) and [drainage](gwf.qmd#drainage)
boundaries can also be used for lateral subsurface flow. By default, lateral subsurface flow
can only flow into a river (slope based). By including the river boundary, flow into (from a
river) or out (into a river) of the subsurface water system is a function of the head. The
river and drainage boundaries can be provided as follows in the TOML configuration file:

```toml
[model]
river_subsurface_exchange_head_based__flag = true # optional river boundary, default is false
drain__flag = true # optional drainage boundary, default is false
```
