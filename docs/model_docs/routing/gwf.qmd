...
Single layer groundwater flow in an unconfined aquifer requires the four following
components:
...
The unconfined aquifer, constanthead boundary and other boundaries (e.g. river and drainage)
are described in more detail below. Which functionality of these components is supported by
...
will vary roughly between $0.05$ (clay) and $0.45$ (peat) (Johnson, 1967). For the model
type `sbm_gwf`, specific yield for a falling water table is defined by the difference
between saturated soil water content and soil water content at field capacity (`SBM` `soil`
model parameters), for a rising water table the specific yield is based on the fillable
porosity, see also the following
[section](..\land\landhydrology_sbm.qmd#water-table-change).
...
$$
    S \frac{\partial h}{\partial t} =  \frac{\partial}{\partial x} \left(kH \frac{\delta h}{\delta x}\right) + Q
$$

where $\SIb{S}{m\ m^{-1}}$ is storativity (or specific yield), $\SIb{h}{m}$ is hydraulic
...
$$
    S_i \frac{h_{i}^{t+1} - h_i^{t}}{\Delta t} \Delta x  \Delta y = -C_{i-1}  (h_{i-1}^{t} - h_i^{t}) - C_i  (h_{i+1}^{t} - h_i^{t}) + Q_i \Delta x  \Delta y
$$

where $i$ is the cell index, $t$ is time, $\Delta t$ is the time step size, $\SIb{\Delta
x}{m}$ is the cell length in the $x$ direction and $\SIb{\Delta y}{m}$ is the cell length in
the $y$ direction, $C_{i-1}$ is the intercell conductance between cell $i-1$ and $i$ and
$C_i$ is the intercell conductance between cell $i$ and $i+1$.
...
where $l_i$ is the length in cell $i$ ($0.5 \Delta x_i$), $l_{i+1}$ is the length in cell
$i+1$ ($0.5 \Delta x_{i+1}$) and $H$, $k$, and $w$ as previously defined. For an unconfined
aquifer the intercell conductance is computed by using the "upstream saturated fraction" as
the MODFLOW documentation calls it. In this approach, the saturated thickness of a
cell-to-cell is approximated using the cell with the highest head. This results in a
consistent overestimation of the saturated thickness, but it avoids complexities related
with cell drying and rewetting, such as having to define a "wetting threshold" or a "wetting
factor". See also the documentation for MODFLOW-NWT (Niswonger et al., 2011) or MODFLOW6
(Langevin et al., 2017) for more background information. For more background on drying and
rewetting, see for example McDonald et al. (1991). For an unconfined aquifer with a
`uniform` `conductivity_profile` the fully saturated intercell conductance is computed once
and scaled each time step by degree of saturation based on the saturated thickness of the
cell with the highest head. For an unconfined aquifer with an `exponential`
`conductivity_profile` the intercell conducantce is computed each time step using the degree
of saturation based on the saturated thickness of the cell with the highest head.

For the finite difference formulation, there is only one unknown, $h_i^{t+1}$. Reshuffling
terms:

$$
h_i^{t+1} = h_i^t + (C_{i-1}  (h_i^{t} - h_{i-1}^{t}) + C_i  (h_{i+1}^{t} - h_i^{t}) + Q_i \Delta x  \Delta y) \frac{Δt}{S_i \Delta x  \Delta y}
$$
...
where $\SIb{\Delta t}{d}$ is the stable time step size, $\SIb{k}{m^2\ d^{-1}}$ is the
horizontal hydraulic conductivity, $\SIb{H}{m}$ is the saturated thickness of the aquifer,
$\alpha$ is a coefficient to enhance the stability of the simulation and $\Delta x$ and
$\Delta y$ as previously defined. For each cell $\frac{\Delta x  \Delta y S}{k H}$ is
calculated, the minimum of these values is determined, and multiplied by $\alpha$, to get
the stable time step size. The stability coefficient $\alpha$ (default = 0.25 as recommended
by Chu and Willis (1984)) can be set as follows in the TOML file:
...
[here](../parameters_routing.qmd#input-gwf-unconfined). Hydraulic head $\SIb{h}{m}$ is an
output and state [variable](../parameters_routing.qmd#state-gwf-unconfined).
...
Note that specific yield is not an input parameter and depends on the `SBM` `soil` model as
...
$$
  ...
  \begin{align*}
    \begin{cases}
      C_i \min \left\{\subtext{h}{riv} - \subtext{B}{riv}, \subtext{h}{riv} - h\right\} &\text{ if }\quad \subtext{h}{riv} > h \\
      C_e (\subtext{h}{riv} - h) &\text{ if }\quad \subtext{h}{riv} \le h
    \end{cases}
  \end{align*}
$$

where $\SIb{\subtext{Q}{riv}}{L^3 T^{-1}}$ is the exchange flux from river to aquifer,
$\SIb{C_i}{L^2 T^{-1}}$ is the river bed infiltration conductance, $\SIb{C_e}{L^2 T^{-1}}$
is the river bed exfiltration conductance, $\SIb{\subtext{B}{riv}}{L}$ the bottom of the
river bed, $\SIb{\subtext{h}{riv}}{L}$ is the river stage and $\SIb{h}{L}$ is the hydraulic
head in the river cell.
...
$$
\subtext{Q}{drain} = \subtext{C}{drain} \min(0, \subtext{h}{drain} - h)
$$
...
$\SIb{\subtext{h}{drain}}{L}$ is the drain elevation and $\SIb{h}{L}$ is the hydraulic head
in the cell with drainage.
...
with $\SIb{C_{hb}}{L^2\ T^{-1}}$ the conductance of the head boundary, $\SIb{h_{hb}}{L}$
the head of the head boundary and $h$ the head of the aquifer cell.

The head boundary flux $Q_{hb}$ is an output variable, and is used to update the total flux
in a cell where this type of boundary occurs. The parameter Head $h_{hb}$ can be
...
::: {.callout-note}
The boundary fluxes are checked, in case of a dry aquifer cell a negative flux is not
allowed.
:::
