...
- the addition of land surface temperature (LST) computations.
...
`allocation` and land surface temperature (`land_surface_temperature`). The sections below
describe the working of the Land Hydrology `SBM` model in more detail.
...
water bodies) the potential reference evapotranspiration rate is used (multiplied by the
...
evaporation and the available water in respectively the unsaturated and saturated zone of
the upper soil layer. Available water in the unsaturated and saturated zone is based on the
effective porosity (difference between saturated and residual water content) and drainable
porosity (difference between water content at saturation and field capacity). The
evaporation (both unsaturated and saturated) decreases linearly with decreasing water
availability, as indicated by the figure below.
...
$\alpha$ (and limited by the available water in saturated zone based on drainable porosity)
...
All infiltrating water enters the unsaturated store first. The soil bucket is split-up in
different layers, by providing the thickness $\SIb{}{mm}$ of the layers in the TOML file.
The following example specifies four layers (from top to bottom) of $50$, $50$, $300$ and
$800$ $\SI{}{mm}$:

```toml
[model]
soil_layer__thickness = [50, 50, 300, 800] # optional, default is [100, 300, 800]
```
...
::: {.callout-important}
When using the "layered_exponential" profile setting, please note that the depth you provide
for $z_\mathrm{layered}$ is being snapped to the closest soil layer, as this profile needs
to be applied to an entire soil layer. If the specific depth is important for your
simulation, please ensure that you have a soil layer present at/near this depth.
:::
...
To convert the groundwater storage change, computed as part of [subsurface flow
routing](..\routing\kinwave.qmd#subsurface-flow-routing) or [unconfined groundwater
flow](..\routing\gwf.qmd#), to the water table change, the specific yield parameter is used.
For a falling water table specific yield is represented by the drainable porosity, the
difference between the saturated water content (required input parameter) and field
capacity. Field capacity, an optional input parameter, is the soil water content after
excess gravitational water has drained away. When this parameter is not provided field
capacity is computed as the water content at a pressure head of -100 cm using the Brooks and
Corey water retention model. For a rising water table, specific yield (dynamic) is
represented by the fillable porosity, the difference between the saturated water content and
water content of unsaturated zone soil layer(s) encountering a water table rise.
...
field capacity (water content at field capacity is an input parameter or computed at a soil
water pressure head of $\SI{-100}{cm}$), $\SIb{\subtext{S}{root-unsat}}{mm}$ is the
unsaturated store in the root zone and $\SIb{\subtext{S}{root-unsat, h3}}{mm}$ is the
unsaturated store in the root zone at the critical soil water pressure head $h_3$, below
this pressure head reduction of root water uptake starts due to drought stress. The net
irrigation demand $\SIb{}{mm\ t^{-1}}$ is the irrigation rate that brings the root zone back
to field capacity, limited by the soil infiltration capacity $\SIb{}{mm\ t^{-1}}$, assuming
that farmers do not apply an irrigation rate higher than the soil infiltration capacity. To
account for limited irrigation efficiency the net irrigation demand is divided by the
irrigation efficiency for non-paddy crops ($\SIb{}{-}$, default is $1.0$), resulting in
gross irrigation demand $\SIb{}{mm\t^{-1}}$. Finally, the gross irrigation demand is limited
by the maximum irrigation rate ($\SIb{}{mm\ t^{-1}}$, default is $\SI{25}{mm\ d^{-1}}$). If
the maximum irrigation rate is applied, irrigation continues at subsequent time steps until
field capacity is reached. Irrigation is added to the amount of water available for
infiltration.
...
$\SIb{\subtext{V}{sw, availability}}{m^3}$ are summed (including reservoir locations limited
by a fixed scaling factor of $0.98$), and the total surface water abstraction is then:

$$
    \subtext{V}{sw, abstraction} = \min (\subtext{V}{sw, demand}, \subtext{V}{sw, availability}).
$$
...
$\SIb{\subtext{V}{gw,demand}}{m^3}$ and availability $\SIb{\subtext{V}{gw, availability}}{m^3}$
are summed, and the total groundwater abstraction is then:

$$
    \subtext{V}{gw, abstraction} = \min(\subtext{V}{gw, demand}, \subtext{V}{gw, availability}).
$$
...
This optional module estimates the land surface temperature (LST)
$\SIb{T_\mathrm{s}}{\degree C}$ based on evapotranspiration $\SIb{ET}{mm\ t^{-1}}$ simulated
by the Land Hydrology `SBM` model and wind speed, air temperature and net radiation as
input. The approach used to estimate $T_\mathrm{s}$ is based on work by Zink et al. (2018)
and Purnamasari et al. (2025).

This module can be enabled by specifying the following in the TOML file:

```toml
[model]
land_surface_temperature__flag = true
```

The energy balance of the land surface is computed as follows:

$$
    R_\mathrm{n} =  LE + H + G
$$

where $\SIb{R_\mathrm{n}}{W\ m^{-2}}$ is net radiation, $\SIb{LE}{W\ m^{-2}}$ is latent heat
flux, $\SIb{H}{W\ m^{-2}}$ is sensible heat flux and $\SIb{G}{W\ m^{-2}}$ is soil heat flux.
The latent heat flux $LE$ is determined by converting evapotranspiration $ET$ to an energy
flux as follows:

$$
    LE =  \lambda \rho_\mathrm{water} ET
$$

where $\SIb{\lambda}{J\ kg^{-1}}$ is latent heat of vaporization approximated by $\lambda =
(2501 - 2.375 T_\mathrm{air})  1000$ using air temperature $\SIb{T_\mathrm{air}}{\degree C}$
and $\rho_\mathrm{water}$ is the density of water ($\SI{1000.0}{kg\ m^{-3}}$). The soil heat
flux $G$ is estimated with a fixed linear coefficient (0.1) applied to net radiation
$R_\mathrm{n}$. With $R_\mathrm{n}$ as input, sensible heat flux $H$ can be estimated using
the energy balance equation. LST can be computed as follows:

$$
    T_\mathrm{s} = \frac{H r_\mathrm{air}}{\rho_\mathrm{air} c_\mathrm{p}} + T_\mathrm{air}
$$

where $\SIb{r_\mathrm{air}}{s\ m^{-1}}$ is aerodynamic resistance, $\SIb{c_\mathrm{p}}{J\
kg^{-1}\ K^{-1}}$ is the specific heat of air ($\SI{1005.0}{J\ kg^{-1}\ K^{-1}}$) and
$\SIb{\rho_\mathrm{air}}{kg\ m^{-3}}$ is the density of air ($\SI{1.225}{kg\ m^{-3}}$).

The aerodynamic resistance $r_\mathrm{air}$ that controls the vapour and heat transfer is
computed based on Thom’s equation (Thom, 1975):

$$
    r_\mathrm{air} = \frac{\ln(\frac{z_\mathrm{m} - d}{z_\mathrm{0m}}) \ln(\frac{z_\mathrm{h} - d}{z_\mathrm{0h}})}{k^2 u_\mathrm{z}}
$$

where $\SIb{d}{m}$ is the zero plane displacement height, $\SIb{z_\mathrm{m}}{m}$ is the
height of wind measurement, $\SIb{z_\mathrm{h}}{m}$ is the height of humidity measurement,
$\SIb{z_\mathrm{0m}}{m}$ is the roughness length for momentum transfer,
$\SIb{z_\mathrm{0h}}{m}$ is the roughness length for heat transfer, $\SIb{u_\mathrm{z}}{m\
s^{-1}}$ is the wind speed at the wind speed measurement height $z_\mathrm{m}$ and $k$ is
the von Kármán constant ($k = 0.41$). It is assumed that the heights of measurements for
wind and humidity are equal. The wind speed is constrained to a minimum value of
$\SI{0.5}{m\ s^{-1}}$ to account for vapour exchange on the surface induced by air buoyancy
and boundary layer instability effects during low wind conditions (Allen et al., 1998).

It is assumed that the input wind speed is consistent with $z_\mathrm{0m}$ and $d$ of the
surface terrain (and not based on "open-terrain" (short grass) as observed typically by
meteorological stations). The input wind speed reference height
`land_surface_wind__speed_reference_height` should be set in the TOML file as follows:

```toml
[model]
land_surface_wind__speed_reference_height = 10.0
```

This reference height should be greater than the sum of $d$ and $z_\mathrm{0m}$ to convert
the wind speed (assuming a logarithmic profile) properly to the reference height $z$ ($z =
z_\mathrm{m} = z_\mathrm{h}$) used for the computation of $r_\mathrm{air}$. The reference
height $z$ is set equal the input parameter `atmosphere_bottom_air__above_ground_height`
(land surface "skin" layer height above the ground), with a minimum of 2 m, consistent with
the FAO (Allen et al., 1998) standardized height for wind speed, temperature and humidity.

The list of input parameters for LST that can be provided through the TOML file can be found
[here](../parameters_landhydrology_sbm.qmd#land-surface-temperature). The required wind
speed, air temperature and net radiation forcing inputs are listed
[here](../parameters_landhydrology_sbm.qmd#atmospheric-forcing).

## References
+ Allen, R. G., Pereira, L. S., Raes, D., an Smith, M., 1998, Crop evapotranspiration -
...
+ Purnamasari, D., Teuling, A. J., and Weerts, A. H., 2025, Identifying irrigated areas
  using land surface temperature and hydrological modelling: application to the Rhine basin,
  Hydrol. Earth Syst. Sci., 29, 1483–1503, https://doi.org/10.5194/hess-29-1483-2025.
...
+ Thom, A. S., 1975, Momentum, mass and heat exchange of plant communities, Vegetation and
  the Atmosphere, 4, 57–109.
...
+ Zink, M., Mai, J., Cuntz, M., and Samaniego, L., 2018, Conditioning a hydrologic model using
  patterns of remotely sensed land surface temperature. Water Resources Research, 54,
  2976–2998. https://doi.org/10.1002/2017WR021346.
