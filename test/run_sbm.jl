...
@test String(read(model.writer.csv_path)) ==
    ...
    2000-01-01T00:00:00,6.007649126299232,4.36446769468067e7,2.3279826641082764,2.3279826641082764,0.02568757752972333,0.00635003022092051,0.00405227570936728,0.002556030233710086,0.0003866259942806916,0.007978020751420236,0.0014150934799747792,0.09113868011976184,0.0015864473330576105,0.0009009895829980564,0.0009794286526124923,0.0005269959322339197,0.0018105957228171281,0.0015987545596470969,0.0014458161216137225,3.0599035141528304,0.592756803639867,3.731898835633116,1.0036622323643711,-0.0571181258953665
    """
...
@testset "overland flow" begin
    ...
    @test sum(q) ≈ 344.45053820126947
    @test q[26625] ≈ 0.0
    @test q[39308] ≈ 0.0
    @test q[network.land.order[end]] ≈ 2.0555467025736123e-6
end

@testset "river flow" begin
    q = model.lateral.river.q_av
    @test sum(q) ≈ 2581.1795255921666
    @test q[4061] ≈ 0.0016541758124978179
    @test q[5617] ≈ 9.065258610407504
    @test q[network.river.order[end]] ≈ 0.002583486681184548
end
