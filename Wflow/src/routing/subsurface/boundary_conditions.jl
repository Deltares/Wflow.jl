...
function flux!(
    ...
    for (idx, index) in enumerate(indices)
        ...
        stage = gwf_river_model.variables.stage[idx]
        if stage > head
            max_infiltration_flux = gwf_river_model.variables.storage[idx] / dt
            cond = gwf_river_model.parameters.infiltration_conductance[idx]
            delta_head = min(stage - gwf_river_model.parameters.bottom[idx], stage - head)
            flux = min(cond * delta_head, max_infiltration_flux)
        else
            cond = gwf_river_model.parameters.exfiltration_conductance[idx]
            delta_head = stage - head
            flux = check_flux(cond * delta_head, subsurface_flow_model, index)
        end
        gwf_river_model.variables.flux[idx] = flux
        subsurface_flow_model.variables.q_net_bnds[index] += flux
        gwf_river_model.variables.storage[idx] -= dt * flux
        gwf_river_model.variables.flux_cumulative[idx] += dt * flux
    end
    return nothing
end
...
function flux!(
    ...
    for (idx, index) in enumerate(indices)
        cond = drainage_model.parameters.conductance[idx]
        delta_head = min(
            0,
            drainage_model.parameters.elevation[idx] -
            subsurface_flow_model.variables.head[index],
        )
        flux = check_flux(cond * delta_head, subsurface_flow_model, index)
        drainage_model.variables.flux[idx] = flux
        drainage_model.variables.flux_cumulative[idx] += flux * dt
        subsurface_flow_model.variables.q_net_bnds[index] += flux
    end
    return nothing
end
...
function flux!(
    ...
    for (idx, index) in enumerate(indices)
        cond = headboundary.parameters.conductance[idx]
        delta_head =
            headboundary.variables.head[idx] - subsurface_flow_model.variables.head[index]
        flux = check_flux(cond * delta_head, subsurface_flow_model, index)
        headboundary.variables.flux[idx] = flux
        headboundary.variables.flux_cumulative[idx] += flux * dt
        subsurface_flow_model.variables.q_net_bnds[index] += flux
    end
    return nothing
end
...
function flux!(
    ...
    for (idx, index) in enumerate(indices)
        flux = check_flux(
            recharge_model.variables.rate[idx] *
            subsurface_flow_model.parameters.area[index],
            subsurface_flow_model,
            index,
        )
        recharge_model.variables.flux[idx] = flux
        recharge_model.variables.flux_cumulative[idx] += flux * dt
        subsurface_flow_model.variables.q_net_bnds[index] += flux
    end
    return nothing
end
...
function flux!(
    ...
    for (idx, index) in enumerate(indices)
        flux = check_flux(
            well_model.variables.volumetric_rate[idx],
            subsurface_flow_model,
            index,
        )
        well_model.variables.flux[idx] = flux
        well_model.variables.flux_cumulative[idx] += flux * dt
        subsurface_flow_model.variables.q_net_bnds[index] += flux
    end
    return nothing
end
...
function update_river_storage_stage!(
    ...
    for idx in eachindex(gwf_river_model.variables.stage)
        gwf_river_model.variables.stage[idx] =
            river_flow_model.variables.h[idx] + gwf_river_model.parameters.bottom[idx]
        gwf_river_model.variables.storage[idx] = river_flow_model.variables.storage[idx]
    end
    return nothing
end
