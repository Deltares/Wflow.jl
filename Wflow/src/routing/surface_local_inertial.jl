...
function get_inflow_reservoir(river_flow::LocalInertialRiverFlow, src_edge::Vector{Int})
    q_in = sum_at(river_flow.variables.q, src_edge)
    if !isnothing(river_flow.floodplain)
        q_in += sum_at(river_flow.floodplain.variables.q, src_edge)
    end
    return q_in
end
...
get_inflow_reservoir(
    ...
    inds::Vector{Int},
) = overland_flow.variables.q_av[inds] .+ overland_flow.variables.to_river[inds]
...
get_inflow_reservoir(
    ...
    (subsurface_flow.variables.ssf[inds] .+ subsurface_flow.variables.to_river[inds]) ./
...
function local_inertial_river_update!(
    ...
    river_v = river_flow.variables
    river_p = river_flow.parameters

    river_v.q0 .= river_v.q
    if !isnothing(river_flow.floodplain)
        river_flow.floodplain.variables.q0 .= river_flow.floodplain.variables.q
    end
    ...
    if !isnothing(river_flow.floodplain)
        floodplain_p = river_flow.floodplain.parameters
        floodplain_v = river_flow.floodplain.variables
    ...
    for v in eachindex(inds_reservoir)
        ...
        q_in = get_inflow_reservoir(river_flow, edges_at_node.src[i])
        ...
        update_reservoir!(reservoir, v, net_inflow, dt, dt_forcing)
...
function update_river_flow!(
    ...
    if !isnothing(river_flow.floodplain)
        river_flow.floodplain.variables.q_av .= 0.0
    end
    set_flow_vars!(river_flow)

    dt = tosecond(clock.dt)
    t = 0.0
    while t < dt
        dt_s = stable_timestep(river_flow, flow_length)
        dt_s = check_timestepsize(dt_s, t, dt)
        local_inertial_river_update!(river_flow, domain, dt_s, dt, update_h)
        t += dt_s
    end
    average_flow_vars!(river_flow, dt)
    average_reservoir_vars!(reservoir, dt)

    if !isnothing(river_flow.floodplain)
        river_flow.floodplain.variables.q_av ./= dt
        river_flow.variables.q_channel_av .= river_flow.variables.q_av
        river_flow.variables.q_av .=
            river_flow.variables.q_channel_av .+ river_flow.floodplain.variables.q_av
    end
...
"""
    stable_timestep(river_flow::LocalInertialRiverFlow, flow_length::Vector{Float64})
    stable_timestep(overland_flow::LocalInertialOverlandFlow, parameters::LandParameters)

Compute a stable timestep size for the local inertial approach, based on Bates et al. (2010).

dt = cfl * (Δx / sqrt(g max(h))
"""
function stable_timestep(river_flow::LocalInertialRiverFlow, flow_length::Vector{Float64})
    dt_min = Inf
    (; cfl) = river_flow.timestepping
    (; n) = river_flow.parameters
    (; h) = river_flow.variables
...
function stable_timestep(
    ...
    dt_min = Inf
    (; cfl) = overland_flow.timestepping
    (; n) = overland_flow.parameters
    (; x_length, y_length, river_location) = parameters
    (; h) = overland_flow.variables
    @batch per = thread reduction = ((min, dt_min),) for i in 1:(n)
...
function update_bc_runoff!(
    ...
    @. overland_flow.boundary_conditions.runoff =
        net_runoff / 1000.0 * area / dt + net_runoff_river * area * 0.001 / dt
    overland_flow.boundary_conditions.runoff[river_indices] .+=
...
function update_inflow!(
    ...
    (; inflow_subsurface) = reservoir.boundary_conditions
...
"""
function set_flow_vars!(overland_flow::LocalInertialOverlandFlow)
    (; qx_av, qy_av) = overland_flow.variables
    qx_av .= 0.0
    qy_av .= 0.0
    return nothing
end
...
"""
function average_flow_vars!(overland_flow::LocalInertialOverlandFlow, dt::Float64)
    (; qx_av, qy_av) = overland_flow.variables
    qx_av ./= dt
    qy_av ./= dt
    return nothing
...
function update_overland_flow!(
    ...
    update_index_hq!(reservoir, clock)
    set_flow_vars!(river_flow)
    set_flow_vars!(overland_flow)

    dt = tosecond(clock.dt)
    t = 0.0
    while t < dt
        dt_river = stable_timestep(river_flow, flow_length)
        dt_land = stable_timestep(overland_flow, parameters)
        dt_s = min(dt_river, dt_land)
        dt_s = check_timestepsize(dt_s, t, dt)

        local_inertial_update_fluxes!(overland_flow, domain, dt_s)
        update_inflow_reservoir!(overland_flow, reservoir, domain)
        local_inertial_river_update!(river_flow, domain, dt_s, dt, update_h)
        local_inertial_update_water_depth!(overland_flow, river_flow, domain, dt_s)

        t += dt_s
    end
    average_flow_vars!(river_flow, dt)
    average_flow_vars!(overland_flow, dt)
    average_reservoir_vars!(reservoir, dt)
