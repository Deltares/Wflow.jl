...
@with_kw struct RiverFlowBC{R}
    ...
    inwater::Vector{Float64} = zeros(length(external_inflow))                          # Lateral inflow [m³ s⁻¹]
    actual_external_abstraction_av::Vector{Float64} = zeros(length(external_inflow))   # Actual abstraction from external negative inflow [m³ s⁻¹]
    abstraction::Vector{Float64} = zeros(length(external_inflow))                      # Abstraction (computed as part of water demand and allocation) [m³ s⁻¹]
        ...
        threaded_foreach(eachindex(order_of_subdomains[k]); basesize = 1) do i
            ...
            for (n, v) in zip(subdomain_indices[m], order_subdomain[m])
                ...
                if external_inflow[v] < 0.0
                    ...
                    # [m³ s⁻¹] += [m³ s⁻¹]
                    actual_external_abstraction_av[v] += _abstraction
                    # [m² s⁻¹] = [m³ s⁻¹] / [m]
...
function update!(model::KinWaveRiverFlow, domain::Domain, clock::Clock)
    ...
    # [s^3/5 m^1/5] =  [s^3/5 m^-1/5] * ([m] + [m])^2/5
