...
@kwdef struct ParameterMetadata{
    ...
    dimname::N = nothing
    tags::Vector{Symbol} = []
    _lens_string::String = string(lens)
...
function metadata_from_lens_string(
    ...
    # _lens_string format is "(@o _.path)" — 6 char prefix, 1 char suffix
    target_len = ncodeunits(lens_string)
    for metadata_candidate in values(standard_name_map)
        ls = metadata_candidate._lens_string
        if ncodeunits(ls) == target_len + 7
            if SubString(ls, 7, ncodeunits(ls) - 1) == lens_string
