...
function mask_transport_capacity(transport_capacity, reservoirs, rivers, dt)
    ...
    elseif reservoirs
        # 1e9 t dt⁻¹
        tc = to_SI(1.0e9, TON_PER_DT; dt_val = dt)
    else
...
function transport_capacity_yalin(
    ...
    alphay = delta * 2.45 / (1.0e-3 * density)^(2 // 5) * sqrt(0.06)
    if q > 0.0 && alphay != 0.0
        TC = (
            width / q *
                (density - 1000) *
...
function transportability_yalin_differentiation(
    ...
    dclay = max(delta / median_diameter_clay - 1, 0.0)
    dsilt = max(delta / median_diameter_silt - 1, 0.0)
    dsand = max(delta / median_diameter_sand - 1, 0.0)
    dsagg = max(delta / median_diameter_small_aggregates - 1, 0.0)
    dlagg = max(delta / median_diameter_large_aggregates - 1, 0.0)
...
function transport_capacity_yalin_differentiation(
    ...
    TCb = 2.45 * sqrt(0.06) / (density / 1.0e3)^(2 // 5)
