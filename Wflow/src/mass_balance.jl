...
function MassBalance(::LateralSSFModel, n::Int)
    mass_balance = MassBalance(; n, storage_prev = [], zi_prev = zeros(n))
    return mass_balance
end

function MassBalance(::GroundwaterFlowModel{A}, n::Int) where {A <: UnconfinedAquiferModel}
...
function watertable_prev!(
    ...
    water_balance.head_prev .= subsurface_flow_model.aquifer.variables.head
end
...
function compute_land_hydrology_balance!(model::AbstractModel{<:SbmGwfModel})
    ...
    boundaries_flow_in, boundaries_flow_out =
        sum_boundary_fluxes(subsurface_flow; exclude = RechargeModel)
...
function compute_flow_balance!(
    ...
    (; ssfin, ssf, exfiltwater, zi) = subsurface_flow_model.variables
    (; specific_yield_dyn) = subsurface_flow_model.parameters
    (; recharge) = subsurface_flow_model.boundary_conditions
...
`GroundwaterFlowModel` in an unconfined aquifer. Errors for subsurface flow constant head
...
function compute_flow_balance!(
    ...
    (; head, q_in_av, q_out_av, exfiltwater) = subsurface_flow_model.aquifer.variables
    (; area, specific_yield_dyn) = subsurface_flow_model.aquifer.parameters

    n = length(head_prev)
    flux_in = zeros(n)
    flux_out = zeros(n)
    flux_in, flux_out = sum_boundary_fluxes(subsurface_flow_model)
    ...
    index_const_head = subsurface_flow_model.constanthead.index
