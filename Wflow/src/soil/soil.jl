                ...
                for k in 1:maximum_number_of_layers
                    if (rooting_depth[i] - cumulative_layer_depth[i][k]) >=
                       actual_layer_thickness[i][k]
                        ...
                        rootfraction[k, i] =
                            max(rooting_depth[i] - cumulative_layer_depth[i][k], 0.0) /
                            rooting_depth[i]
                    end
                end
            end
...
function soil_evaporation!(soil_model::SbmSoilModel)
    ...
    threaded_foreach(1:n; basesize = 1000) do i
        ...
        soilevapunsat = min(soilevapunsat, v.unsaturated_layer_depth[i][1])
        # Update the additional atmospheric demand
        potsoilevap -= soilevapunsat
        v.unsaturated_layer_depth[i] = setindex(
            v.unsaturated_layer_depth[i],
            v.unsaturated_layer_depth[i][1] - soilevapunsat,
            1,
        )
...
function transpiration!(soil_model::SbmSoilModel, dt::Float64)
    ...
    threaded_foreach(1:n; basesize = 250) do i
        ...
        for k in 1:v.n_unsatlayers[i]
            # the root fraction is valid for the root length in a soil layer, if water_table_depth decreases
        ...
        for k in 1:v.n_unsatlayers[i]
            ...
            maxextr = v.unsaturated_layer_depth[i][k] * availcap
            actevapustore_layer =
                min(alpha * rootfraction_unsat_scaled * potential_transpiration[i], maxextr)
            unsaturated_layer_depth = v.unsaturated_layer_depth[i][k] - actevapustore_layer
...
function capillary_flux!(soil_model::SbmSoilModel)
    ...
    threaded_foreach(1:n; basesize = 1000) do i
        if v.n_unsatlayers[i] > 0
            ...
            if v.water_table_depth[i] > rooting_depth[i]
                capflux =
                    maxcapflux * pow(
            ...
            for k in v.n_unsatlayers[i]:-1:1
                toadd = min(
                    ...
                    max(
                        v.unsaturated_layer_thickness[i][k] *
                        (p.theta_s[i] - p.theta_r[i]) - v.unsaturated_layer_depth[i][k],
                        0.0,
                    ),
                )
                v.unsaturated_layer_depth[i] = setindex(
                    v.unsaturated_layer_depth[i],
                    v.unsaturated_layer_depth[i][k] + toadd,
                    k,
                )
                netcapflux -= toadd
...
function leakage!(soil_model::SbmSoilModel)
    ...
    threaded_foreach(1:n; basesize = 1000) do i
        ...
        deeptransfer = min(v.drainable_water_depth[i], deepksat)
    ...
    if water_table_depth < zi_prev
        for k in n_unsatlayers:n_unsatlayers_prev
            ...
            if isnan(unsaturated_layer_thickness[k])
                ...
                unsaturated_layer_depth =
                    setindex(unsaturated_layer_depth, f * unsaturated_layer_depth[k], k)
            end
        end
    else
...
function update_ustorelayerdepth!(soil_model::SbmSoilModel, subsurface_flow)
    ...
    water_table_depth = get_water_depth(subsurface_flow) * 1000.0 # convert from [m] to [mm]
...
model, resulting in a change in water table depth and an exfiltration rate `exfiltration_water`.
...
function update_soil_water_storage!(soil_model::SbmSoilModel, external_models::NamedTuple)
    ...
    exfiltration_saturated_water = subsurface_flow.variables.exfiltration_water * 1000.0 # convert from [m] to [mm]
    rooting_depth = get_rootingdepth(soil_model)
