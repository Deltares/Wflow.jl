...
@with_kw struct AllocationRiverVariables
    n::Int
    act_surfacewater_abst::Vector{Float64} = zeros(n)       # actual surface water abstraction [m s⁻¹]
    ...
    nonirri_returnflow::Vector{Float64} = zeros(n)          # return flow from non irrigation [m s⁻¹]
end
