...
@with_kw struct DemandVariables
    n::Int
    irrigation_demand_gross::Vector{Float64} = zeros(n)        # irrigation gross demand [mm Δt⁻¹]
    non_irrigation_demand_gross::Vector{Float64} = zeros(n)     # non-irrigation gross demand [mm Δt⁻¹]
...
@with_kw struct AllocationRiverVariables
    n::Int
    actual_surfacewater_abstraction::Vector{Float64} = zeros(n)        # actual surface water abstraction [mm Δt⁻¹]
    actual_surfacewater_abstraction_volume::Vector{Float64} = zeros(n)    # actual surface water abstraction [m³ Δt⁻¹]
    available_surfacewater::Vector{Float64} = zeros(n)       # available surface water [m³]
    non_irrigation_returnflow::Vector{Float64} = zeros(n)           # return flow from non irrigation [mm Δt⁻¹]
end
...
@with_kw struct AllocationLandVariables
    n::Int
    surfacewater_allocation::Vector{Float64} = zeros(n)           # allocation from surface water [mm Δt⁻¹]
    actual_groundwater_abstraction::Vector{Float64} = zeros(n)         # actual groundwater abstraction [mm Δt⁻¹]
    actual_groundwater_abstraction_volume::Vector{Float64} = zeros(n)     # actual groundwater abstraction [m³ Δt⁻¹]
    available_groundwater::Vector{Float64} = zeros(n)        # available groundwater [m³]
    groundwater_allocation::Vector{Float64} = zeros(n)            # allocation from groundwater [mm Δt⁻¹]
    irrigation_allocation::Vector{Float64} = zeros(n)                   # allocated water for irrigation [mm Δt⁻¹]
    non_irrigation_allocation::Vector{Float64} = zeros(n)                # allocated water for non-irrigation [mm Δt⁻¹]
...
function surface_water_allocation_local!(
    ...
    for (i, index_river) in enumerate(indices_river)
        if index_river > 0
            ...
            actual_surfacewater_abstraction_volume[index_river] = abstraction_vol
    ...
    for i in eachindex(inds_river)
        ...
        for j in inds_river[i]
            actual_surfacewater_abstraction_volume[j] +=
                frac_abstract_sw * available_surfacewater[j]
...
function groundwater_allocation_local!(
    ...
    for i in eachindex(groundwater_demand)
        ...
        if !reservoir_coverage[i]
            ...
            actual_groundwater_abstraction_volume[i] = abstraction_vol
    ...
    for i in eachindex(inds_river)
        ...
        for j in inds_land[i]
            actual_groundwater_abstraction_volume[j] +=
                frac_abstract_gw * available_groundwater[j]
            actual_groundwater_abstraction[j] =
                1000.0 * (actual_groundwater_abstraction_volume[j] / area[j])
...
function update_water_allocation_model!(
    ...
    @. abstraction = actual_surfacewater_abstraction_volume / dt
    ...
    if !isnothing(reservoir)
        ...
        @. reservoir.variables.storage -=
            actual_surfacewater_abstraction_volume[inds_reservoir]
    ...
    for i in eachindex(non_irrigation_returnflow)
        if inds_river[i] > 0.0
            ...
            non_irrigation_returnflow[i] = non_irrigation_returnflow[i]
        end
    end
end
