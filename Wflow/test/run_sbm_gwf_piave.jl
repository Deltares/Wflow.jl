...
@testitem "water balance piave water demand (sbm_gwf model)" begin
    ...
    @testset "water balance first timestep" begin
        @test all(e -> abs(e) < 0.3, land_water_balance.error)
        ...
        @test all(e -> abs(e) < 1.0e-9, land_water_balance.error[inds])
        @test all(re -> abs(re) < 1.0e-9, land_water_balance.relative_error[inds])
