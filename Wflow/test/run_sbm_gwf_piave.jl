...
@testitem "water balance piave water demand (sbm_gwf model)" begin
    ...
    @testset "water balance first timestep" begin
        ...
        inds_q = findall(x -> x > 1.0e-30, model.routing.river_flow.variables.q)
        @test all(e -> abs(e) < 1.0e-9, river_water_balance.error[inds_q])
        @test all(re -> abs(re) < 1.0e-9, river_water_balance.relative_error[inds_q])
