@testitem "unit: update_boundary_conditions!" begin
    using Wflow: to_SI, MM, MM_PER_DT
    ...
    atmospheric_forcing = Wflow.AtmosphericForcing(;
        n,
        potential_evaporation = [to_SI(1e-3, MM_PER_DT; dt_val = dt)],
    )
    ...
    interception = Wflow.GashInterceptionModel(;
        ...
        parameters = Wflow.GashParameters(;
            ...
            vegetation_parameter_set = Wflow.VegetationParameters(;
                ...
                cmax = [to_SI(10.0, MM)],
                rootingdepth = [to_SI(10.0, MM)],
                kc = [0.1],
            ),
        ),
        variables = Wflow.InterceptionVariables(;
            n,
            canopy_potevap = [to_SI(4e-3, MM_PER_DT; dt_val = dt)],
    ...
    runoff = Wflow.OpenWaterRunoff(;
        ...
        variables = Wflow.OpenWaterRunoffVariables(;
            n,
            runoff_land = [to_SI(6e-4, MM_PER_DT; dt_val = dt)],
            runoff_river = [to_SI(7e-3, MM_PER_DT; dt_val = dt)],
        ),
        boundary_conditions = Wflow.OpenWaterRunoffBC(;
            n,
            water_flux_surface = [to_SI(7.5e-3, MM_PER_DT; dt_val = dt)],
        ),
    )
    demand = Wflow.Demand(;
    ...
    allocation = Wflow.AllocationLand(;
        ...
        variables = Wflow.AllocationLandVariables(;
            n,
            irri_alloc = [to_SI(2e-2, MM_PER_DT; dt_val = dt)],
        ),
    )
    ...
    @test model.boundary_conditions.potential_transpiration[1] ≈
          to_SI(2e-3, MM_PER_DT; dt_val = dt)
    @test model.boundary_conditions.potential_soilevaporation[1] ≈
          to_SI(2.9e-4, MM_PER_DT; dt_val = dt)
    @test model.boundary_conditions.water_flux_surface[1] ≈
          to_SI(1.99e-2, MM_PER_DT; dt_val = dt)
end

@testitem "unit: unsaturated_zone_flow!" begin
    using Wflow: to_SI, MM, MM_PER_DT, Unit
    PER_MM = Unit(; mm = -1)
    using StaticArrays: SVector
    dt = 86400.0
    include("testing_utils.jl")
    ...
    model = init_sbm_soil_model(
        ...
        ustorelayerthickness = [
            to_SI.(SVector((50.0, 100.0, 50.0, 200.0, 800.0, 250.0)), Ref(MM)),
        ],
        ustorelayerdepth = [to_SI.(SVector(8.3, 16.6, 7.7, 36.5, 190.7, 59.2), Ref(MM))],
        n_unsatlayers = [6],
        theta_s = [0.5],
        theta_r = [0.07],
        kv_profile = Wflow.KvExponential(
            [to_SI(2530.0, MM_PER_DT; dt_val = dt)],
            [to_SI(1e-3, PER_MM)],
        ),
        kvfrac = [SVector((0.9, 0.8, 0.9, 0.8, 0.9, 0.8))],
        c = [SVector((9.3, 9.1, 8.9, 8.7, 8.5, 8.3))],
        infiltsoilpath = [to_SI(1.5, MM_PER_DT; dt_val = dt)],
    )
    ...
    @test model.variables.transfer[1] ≈ to_SI(4.437857871332029, MM_PER_DT; dt_val = dt)
end
...
@testitem "unit: soil_evaporation!" begin
    ...
    @test model.variables.soilevapsat[1] ≈ 0.40360465116279065
    @test model.variables.soilevap[1] ≈ 2.2855813953488378
    @test model.variables.drainable_waterdepth[1] ≈ 99.5963953488372
end
...
@testitem "unit: transpiration!" begin
    ...
    model = init_sbm_soil_model(
        ...
        h3_low = [to_SI(-100.0, CM)],
        potential_transpiration = [to_SI(1.2, MM_PER_DT; dt_val = dt)],
        n_unsatlayers = [6],
        zi = [to_SI(75.0, MM)],
        ustorelayerthickness = [
            to_SI.(SVector((50.0, 100.0, 50.0, 200.0, 800.0, 250.0)), Ref(MM)),
        ...
        ustorelayerdepth = [to_SI.(SVector(8.3, 16.6, 7.7, 36.5, 190.7, 59.2), Ref(MM))],
        rootingdepth = [to_SI(106.8, MM)],
        rootfraction = [SVector(0.125, 0.25, 0.125, 0.5, 0.0, 0.0)],
        act_thickl = [to_SI.(SVector(50.0, 100.0, 50.0, 200.0, 800.0, 573.3), Ref(MM))],
        sumlayers = [
            to_SI.(SVector(0.0, 50.0, 150.0, 200.0, 400.0, 1200.0, 1773.3), Ref(MM)),
        ],
        c = [SVector((9.3, 9.1, 8.9, 8.7, 8.5, 8.3))],
        theta_s = [0.5],
        theta_r = [0.07],
        ...
        h4 = [to_SI(-15849.0, CM)],
        alpha_h1 = [1.0],
        rootdistpar = [to_SI(-500.0, PER_MM)],
        drainable_waterdepth = [to_SI(100.0, MM)],
    )

    Wflow.transpiration!(model, dt)

    @test model.variables.ae_ustore[1] ≈ to_SI(0.4479021069963573, MM_PER_DT; dt_val = dt)
    @test model.variables.actevapsat[1] ≈ to_SI(0.7520978930036426, MM_PER_DT; dt_val = dt)
    @test model.variables.drainable_waterdepth[1] ≈ to_SI(99.24790210699636, MM)
    @test model.variables.transpiration[1] ≈ to_SI(1.2, MM_PER_DT; dt_val = dt)
end
...
@testitem "unit: capillary_flux!" begin
    ...
    model = init_sbm_soil_model(
        ...
        ustorelayerdepth = [to_SI.(SVector(8.3, 16.6, 7.7, 36.5, 190.7, 59.2), Ref(MM))],
        ae_ustore = [to_SI(1.25, MM_PER_DT; dt_val = dt)],
        ustorecapacity = [to_SI(385.0, MM)],
        drainable_waterdepth = [to_SI(100.0, MM)],
    ...
    @test model.variables.actcapflux[1] ≈ to_SI(1.11392, MM_PER_DT; dt_val = dt)
end

@testitem "unit: update! SbmSoilModel" begin
    using StaticArrays: SVector
    using Wflow: to_SI, MM_PER_DT, MM, Unit
    ...
    model = init_sbm_soil_model(
        ...
        soilthickness = [to_SI(2000.0, MM)],
        soilwatercapacity = [to_SI(1215.5, MM)],
        ustorecapacity = [0.0],
        satwaterdepth = [0.0],
        drainable_waterdepth = [0.0],
        total_soilwater_storage = [0.0],
        excesswater = [to_SI(0.25, MM_PER_DT; dt_val = dt)],
    ...
    @test demand.paddy.variables.h[1] ≈ to_SI(2.0, MM)
    @test model.variables.runoff[1] ≈ to_SI(0.88, MM_PER_DT; dt_val = dt)
    @test model.variables.ustorecapacity[1] ≈ to_SI(58.0, MM)
    @test model.variables.satwaterdepth[1] ≈ to_SI(838.5, MM)
    @test model.variables.drainable_waterdepth[1] ≈ to_SI(487.5, MM)
    @test model.variables.exfiltsatwater[1] ≈ to_SI(2.5, MM_PER_DT; dt_val = dt)
