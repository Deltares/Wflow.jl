...
config = Wflow.Config(
    tomlpath;
    time_starttime = DateTime("2000-01-01T00:00:00"),
    time_endtime = DateTime("2000-02-01T00:00:00"),
    model_cold_start__flag = true, # cold start
    # note that this needs to be relative to the tomlpath
    state_path_output = joinpath(
        dirname(tomlpath),
...
config = Wflow.Config(
    tomlpath;
    time_starttime = DateTime("2000-01-01T00:00:00"),
    time_endtime = DateTime("2000-01-15T00:00:00"),
    model_cold_start__flag = true, # cold start
    # note that this needs to be relative to the tomlpath
    state_path_output = joinpath(
        dirname(tomlpath),
...
config = Wflow.Config(
    tomlpath;
    time_starttime = DateTime("2000-01-15T00:00:00"),
    time_endtime = DateTime("2000-02-01T00:00:00"),
    model_cold_start__flag = false,  # warm start
    state_path_input = joinpath(
        dirname(tomlpath),
        "data/state-test/outstates-moselle-january-1of2.nc",
    ),
    state_path_output = joinpath(
...
config = Wflow.Config(
    tomlpath;
    time_starttime = DateTime("2000-01-14T00:00:00"),
    time_endtime = DateTime("2000-02-01T00:00:00"),
    model_cold_start__flag = false, # warm start
    fews_run__flag = true,
    state_path_input = joinpath(
